...
from ..src.models.quantizer import EMAVectorQuantizer, ResidualEMAVectorQuantizer
...
class QuantizerContractTest(unittest.TestCase):
    ...
    def test_residual_quantizer_returns_per_level_stats(self) -> None:
        """RVQ should expose one index grid and usage bundle per residual level."""
        quantizer = ResidualEMAVectorQuantizer(
            num_embeddings=16,
            embedding_dim=8,
            num_quantizers=2,
        ...
        self.assertEqual(outputs.quantized.shape, latents.shape)
        self.assertEqual(outputs.indices.shape, (2, 2, 4, 3))
        self.assertEqual(outputs.perplexity.shape, (2,))
        self.assertEqual(outputs.active_codes.shape, (2,))
        self.assertEqual(outputs.usage_counts.shape, (2, 16))
        self.assertEqual(outputs.embed_sum.shape, (2, 16, 8))
        self.assertEqual(outputs.restart_candidates.shape[0], 2)
        self.assertEqual(outputs.restart_candidates.shape[2], 8)
