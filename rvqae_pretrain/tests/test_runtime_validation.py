...
def _build_training_args(**overrides):
    """Create a minimal valid RVQAE training-args namespace."""
    payload = {
        "lr": 1e-3,
        "min_lr": 0.0,
        ...
        "w_min": 0.1,
        "w_max": 200.0,
        "enc_conv_channels": [32, 64, 128],
        "enc_conv_strides": [2, 2, 2],
        "patch_size": 1,
        "embed_dim": 128,
        "enc_vit_depth": 2,
        "enc_vit_head": 4,
...
class RuntimeValidationTest(unittest.TestCase):
    ...
    def test_training_args_reject_invalid_rvq_loss_weights(self) -> None:
        """Trainer validation should reject ambiguous RVQ loss weights."""
        with self.assertRaisesRegex(ValueError, "rvq_loss_weights"):
            _validate_training_args(_build_training_args(rvq_num_quantizers=2, rvq_loss_weights=[1.0]))
        with self.assertRaisesRegex(ValueError, "rvq_loss_weights"):
            _validate_training_args(_build_training_args(rvq_num_quantizers=1, rvq_loss_weights=[0.0]))
