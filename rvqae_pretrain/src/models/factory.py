"""Model factory and checkpoint loading APIs for RVQAE pretraining."""
...
from .rvqae import PolyRvqAutoencoder
...
def infer_img_size_from_config(config: dict[str, Any]) -> tuple[int, int]:
    ...
    enc_conv_strides = _parse_int_sequence(config.get("enc_conv_strides"), default=())
    patch_size = int(config.get("patch_size", 4))
    align_stride = max(1, math.prod(int(value) for value in enc_conv_strides) * patch_size)
...
def _parse_float_sequence(value: Any, default: Sequence[float]) -> tuple[float, ...]:
    """Parse one float sequence from config value or fall back to default."""
    if value is None:
...
def build_rvqae_model_from_config(
    config: dict[str, Any],
    device: str | torch.device = "cpu",
    precision: str = "fp32",
) -> PolyRvqAutoencoder:
    """Construct one RVQAE model from config dictionary."""
    img_size = infer_img_size_from_config(config)
    enc_conv_channels = _parse_int_sequence(config.get("enc_conv_channels"), default=())
    enc_conv_strides = _parse_int_sequence(config.get("enc_conv_strides"), default=())
    full_res_head_channels = _parse_int_sequence(config.get("full_res_head_channels"), default=())
    rvq_num_quantizers = int(config.get("rvq_num_quantizers", 1))
    rvq_loss_weights = _parse_float_sequence(
        config.get("rvq_loss_weights"),
        default=tuple(1.0 for _ in range(rvq_num_quantizers)),
    )
    ...
    model = PolyRvqAutoencoder(
        ...
        enc_vit_depth=int(config.get("enc_vit_depth", 8)),
        enc_vit_head=int(config.get("enc_vit_head", 8)),
        enc_vit_mlp_ratio=float(config.get("enc_vit_mlp_ratio", 4.0)),
        dec_vit_depth=int(config.get("dec_vit_depth", 8)),
        dec_vit_head=int(config.get("dec_vit_head", 8)),
        dec_vit_mlp_ratio=float(config.get("dec_vit_mlp_ratio", 4.0)),
        ...
        rvq_num_quantizers=rvq_num_quantizers,
...
def load_rvqae_model(
    weight_path: str | Path,
    config_path: str | Path,
    device: str | torch.device = "cpu",
    precision: str = "fp32",
) -> tuple[PolyRvqAutoencoder, dict[str, Any]]:
    """Load one full RVQAE model from checkpoint and config."""
    config = load_config_any(config_path)
    model = build_rvqae_model_from_config(config, device="cpu", precision="fp32")
...
def _extract_encoder_state_dict(state_dict: dict[str, Any]) -> dict[str, Any]:
    """Extract encoder-only state dict from a full RVQAE checkpoint."""
...
def export_rvqae_components(
    rvqae_ckpt_path: str | Path,
    config_path: str | Path,
    output_dir: str | Path,
    precision: str = "fp32",
) -> Path:
    """Export full-model and component checkpoints from one RVQAE checkpoint."""
    from ..utils.checkpoint import save_checkpoint
    from ..utils.config import dump_yaml_config
    from ..utils.filesystem import ensure_dir

    model, runtime_config = load_rvqae_model(
        weight_path=rvqae_ckpt_path,
    ...
    save_checkpoint(out_dir / "rvqae_best.pth", model.state_dict(), precision=precision)
...
def load_decoder_from_components(
    ...
    """Load one RVQAE model with decoder+quantizer components for code decoding."""
    config = load_config_any(config_path)
    model = build_rvqae_model_from_config(config, device="cpu", precision="fp32")
