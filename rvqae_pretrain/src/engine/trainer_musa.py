"""Training engine for polygon RVQAE pretraining.
...
from ..models.factory import build_rvqae_model_from_config
...
def plot_reconstruction(
    ...
    Args:
        ...
        spatial_icft_recon: ICFT reconstruction from RVQAE output.
...
def _build_model_kwargs(args, img_size: tuple[int, int]) -> dict:
    """Build one shared RVQAE model-kwargs dict from runtime args."""
    return {
        "img_size": img_size,
        "in_chans": 3,
        "enc_conv_channels": args.enc_conv_channels,
        "enc_conv_strides": args.enc_conv_strides,
        "patch_size": args.patch_size,
        "embed_dim": args.embed_dim,
        "enc_vit_depth": args.enc_vit_depth,
        "enc_vit_head": args.enc_vit_head,
        "enc_vit_mlp_ratio": args.enc_vit_mlp_ratio,
        "dec_vit_depth": args.dec_vit_depth,
        "dec_vit_head": args.dec_vit_head,
        "dec_vit_mlp_ratio": args.dec_vit_mlp_ratio,
        "full_res_head_channels": args.full_res_head_channels,
        "codebook_size": args.codebook_size,
        "code_dim": args.code_dim,
        "rvq_num_quantizers": args.rvq_num_quantizers,
        "rvq_loss_weights": args.rvq_loss_weights,
...
def _parse_literal_int_list(value) -> list[int]:
    """Parse a YAML-style integer list passed through CLI."""
...
def _parse_literal_float_list(value) -> list[float]:
    """Parse a YAML-style float list passed through CLI."""
    if isinstance(value, list):
...
def _build_model(args, img_size: tuple[int, int], device: torch.device, dist_ctx: DistContext):
    """Construct RVQAE model and optional DDP wrapper.
    ...
    """
    model = build_rvqae_model_from_config(_build_model_kwargs(args, img_size=img_size), device=device, precision="fp32")
...
def _validate_training_args(args) -> None:
    ...
    if args.enc_vit_depth <= 0:
        raise ValueError(f"`enc_vit_depth` must be > 0, got {args.enc_vit_depth}")
    if args.enc_vit_head <= 0:
        raise ValueError(f"`enc_vit_head` must be > 0, got {args.enc_vit_head}")
    if args.dec_vit_depth <= 0:
        raise ValueError(f"`dec_vit_depth` must be > 0, got {args.dec_vit_depth}")
    if args.dec_vit_head <= 0:
        raise ValueError(f"`dec_vit_head` must be > 0, got {args.dec_vit_head}")
    ...
    if args.rvq_num_quantizers <= 0:
        raise ValueError(f"`rvq_num_quantizers` must be > 0, got {args.rvq_num_quantizers}")
    if len(args.rvq_loss_weights) != int(args.rvq_num_quantizers):
        raise ValueError(
            "`rvq_loss_weights` length must equal `rvq_num_quantizers`: "
            f"got {len(args.rvq_loss_weights)} vs {args.rvq_num_quantizers}"
        )
    if any(float(weight) <= 0 for weight in args.rvq_loss_weights):
    ...
    if len(args.enc_conv_channels) != len(args.enc_conv_strides):
        raise ValueError("`enc_conv_channels` and `enc_conv_strides` must have the same length")
    if any(int(stride) <= 0 for stride in args.enc_conv_strides):
        raise ValueError("All `enc_conv_strides` values must be > 0")
    if any(int(channel) <= 0 for channel in args.enc_conv_channels):
        raise ValueError("All `enc_conv_channels` values must be > 0")
    if any(int(channel) <= 0 for channel in args.full_res_head_channels):
        raise ValueError("All `full_res_head_channels` values must be > 0")
...
def _sync_run_metadata(best_dir: Path, ckpt_dir: Path, run_config: dict, model_config: dict) -> None:
    ...
    _write_json(best_dir / "poly_rvqae_config.json", model_config)
    _write_json(ckpt_dir / "poly_rvqae_config.json", model_config)
...
def _iter_quantizers(model_or_quantizer):
    """Yield one or more EMA codebooks from a model/quantizer wrapper."""
    quantizer = getattr(model_or_quantizer, "quantizer", model_or_quantizer)
    if hasattr(quantizer, "iter_quantizers"):
        return list(quantizer.iter_quantizers())
    return [quantizer]


def _broadcast_quantizer_state(model_or_quantizer, dist_ctx: DistContext) -> None:
    """Broadcast all quantizer buffers from rank 0 to all ranks."""
    if not (dist_ctx.enabled and dist_ctx.world_size > 1):
        return
    for quantizer in _iter_quantizers(model_or_quantizer):
        for buffer in (
            quantizer.codebook,
...
def _register_vq_debug_latent_hook(model_to_save, args) -> tuple[dict, object | None]:
    ...
    def _capture_quantizer_input(module, inputs) -> None:
        ...
        if bool(args.vq_debug_nearest_check):
            debug_quantizer = _iter_quantizers(module)[0]
            debug_state["codebook_snapshot"] = debug_quantizer.codebook.detach().float().clone()
...
def _build_codebook_debug_stats(quantizer) -> dict[str, float]:
    ...
    quantizer = _iter_quantizers(quantizer)[0]
...
def _sync_quantizer_step_state(
    *,
    ...
    """Synchronize RVQ stats and trainer-owned EMA updates across ranks."""
    debug_info = {
        "dead_before": 0,
    ...
    quantizers = _iter_quantizers(quantizer)
    usage_by_level = global_usage_counts if global_usage_counts.ndim == 2 else global_usage_counts.unsqueeze(0)
    embed_by_level = global_embed_sum if global_embed_sum.ndim == 3 else global_embed_sum.unsqueeze(0)
    restart_by_level = outputs.restart_candidates
    if restart_by_level is not None and restart_by_level.ndim == 2:
        restart_by_level = restart_by_level.unsqueeze(0)

    for level, level_quantizer in enumerate(quantizers):
        level_quantizer.apply_ema_update(usage_by_level[level], embed_by_level[level])

        dead_code_threshold = float(level_quantizer.dead_code_threshold)
        dead_code_count = (
            int((level_quantizer.cluster_size < dead_code_threshold).sum().item())
        ...
        level_candidates = None if restart_by_level is None else restart_by_level[level]
        restart_candidates = _gather_restart_candidates(
            level_candidates,
            code_dim=int(level_quantizer.embedding_dim),
            max_per_rank=restart_pool_size_per_rank,
            device=level_quantizer.codebook.device,
            dist_ctx=dist_ctx,
        )
        if debug_enabled:
        ...
        if is_main_process(dist_ctx):
            restarted_dead_codes = level_quantizer.restart_dead_codes(restart_candidates)
        ...
        debug_info["restart_happened"] = debug_info["restart_happened"] or bool(restart_flag.item())
    return debug_info
...
def _accumulate_epoch_metrics(metric_sums: dict[str, float], step_outputs: dict[str, torch.Tensor]) -> None:
    """Accumulate metrics from one model step into running sums."""
    ...
    for name, value in (
        ("perplexity", step_outputs["outputs"].perplexity.detach().float()),
        ("active", step_outputs["outputs"].active_codes.detach().float()),
    ):
        if not torch.is_tensor(metric_sums[name]):
            metric_sums[name] = torch.zeros_like(value)
...
def _reduce_epoch_metrics(
    ...
    *,
    ...
    return {
        name: all_reduce_mean(
            (value / step_count).to(device) if torch.is_tensor(value) else torch.tensor(value / step_count, device=device),
            dist_ctx,
        )
...
def _format_float_vector(value: torch.Tensor, digits: int = 2) -> str:
    """Format one scalar/vector tensor as a compact list."""
...
def _format_int_vector(value: torch.Tensor) -> str:
    """Format one scalar/vector tensor as an integer list."""
    flat = value.detach().float().flatten().cpu().tolist()
...
def _format_epoch_metrics(prefix: str, metrics: dict[str, torch.Tensor]) -> str:
    ...
    return (
        ...
        f"VQ: {metrics['vq'].item():.4f} | Perplexity: {_format_float_vector(metrics['perplexity'])} | "
        f"ActiveCodes: {_format_int_vector(metrics['active'])}"
    )
...
def train_main(args) -> None:
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            if is_main_process(dist_ctx):
                stage_name = "RVQAE" if use_vq else "AE warmup"
            ...
            for step, (batch_tris, lengths) in enumerate(train_loader):
                ...
                if is_main_process(dist_ctx) and step % args.log_interval == 0:
                    ...
                    print(
                        ...
                        f"Perplexity: {_format_float_vector(step_outputs['outputs'].perplexity)}, "
                        f"ActiveCodes: {_format_int_vector(step_outputs['outputs'].active_codes)}"
                        f"{grad_debug_suffix}"
                    )
                if (
                    args.vq_init_debug
                    ...
                    usage_counts_for_topk = usage_counts[0] if usage_counts.ndim == 2 else usage_counts
                    topk = torch.topk(
                        usage_counts_for_topk,
                        k=min(10, int(usage_counts_for_topk.numel())),
                    ).values.tolist()
                    print(
                        ...
                        f"perplexity={_format_float_vector(step_outputs['outputs'].perplexity)} "
                        f"active={_format_int_vector(step_outputs['outputs'].active_codes)} "
                        f"topk_usage={[int(v) for v in topk]}"
                    )

                if args.vq_debug and use_vq and step_outputs["outputs"].usage_counts is not None:
                    usage_counts = step_outputs["outputs"].usage_counts
                    usage_counts_for_debug = usage_counts[0] if usage_counts.ndim == 2 else usage_counts
                    usage_debug = _build_usage_debug_stats(
                        usage_counts_for_debug,
                        topk=args.vq_debug_topk,
                    )
                    active_codes = int(step_outputs["outputs"].active_codes.detach().float().min().item())
                    trigger_reasons = []
                        ...
                        if bool(has_latent_stats.item()):
                            ...
                            if is_main_process(dist_ctx):
                                print(
                                    ...
                                    f"ppl={_format_float_vector(step_outputs['outputs'].perplexity)} "
                                    f"top1={usage_debug['top1_ratio']:.4f} "
                                    f"top{min(args.vq_debug_topk, len(usage_debug['top_counts']))}="
            ...
            if should_eval and is_main_process(dist_ctx) and val_metrics is not None:
                ...
                if current_val_loss < best_val_loss:
                    ...
                    save_checkpoint(best_dir / "rvqae_best.pth", model_to_save.state_dict(), precision=args.checkpoint_dtype)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="RVQAE pretraining trainer")

    parser.add_argument("--train_type", type=str, default="rvqae")
    ...
    parser.add_argument("--enc_conv_channels", type=_parse_literal_int_list, default=[64, 128, 256])
    parser.add_argument("--enc_conv_strides", type=_parse_literal_int_list, default=[2, 2, 1])
    parser.add_argument("--patch_size", type=int, default=2)
    parser.add_argument("--embed_dim", type=int, default=384)
    parser.add_argument("--enc_vit_depth", type=int, default=8)
    parser.add_argument("--enc_vit_head", type=int, default=8)
    parser.add_argument("--enc_vit_mlp_ratio", type=float, default=4.0)

    parser.add_argument("--dec_vit_depth", type=int, default=8)
    parser.add_argument("--dec_vit_head", type=int, default=8)
    parser.add_argument("--dec_vit_mlp_ratio", type=float, default=4.0)
    parser.add_argument("--full_res_head_channels", type=_parse_literal_int_list, default=[])

    parser.add_argument("--codebook_size", type=int, default=8192)
    parser.add_argument("--code_dim", type=int, default=128)
    parser.add_argument("--rvq_num_quantizers", type=int, default=1)
    parser.add_argument("--rvq_loss_weights", type=_parse_literal_float_list, default=[1.0])
...
def run_cli(argv=None) -> None:
    ...
    if "LOCAL_RANK" not in os.environ:
        # Only set MUSA_VISIBLE_DEVICES for non-torchrun single-process launches.
