"""Unified inference pipelines for polygon RVQAE downstream usage."""
...
from ..models.factory import load_decoder_from_components, load_pretrained_encoder, load_rvqae_model
from ..utils.config import load_config_any
from ..utils.precision import autocast_context, normalize_precision


class PolyRvqAePipeline:
    """Full-model RVQAE inference pipeline."""
    ...
    def __init__(
        ...
        self.model, self.config = load_rvqae_model(
    ...
    def triangles_to_images(self, triangles_list: Sequence[np.ndarray]) -> torch.Tensor:
        """Convert triangle arrays to RVQAE input channels."""
    ...
    def reconstruct_triangles(
        ...
        """Reconstruct one triangle batch into real/imaginary maps and RVQ stats."""
        imgs = self.triangles_to_images(triangles_list)
        with autocast_context(self.device, self.precision):
...
class PolyRvqDecodePipeline:
    """Decoder-only RVQ pipeline for code-index decoding."""
