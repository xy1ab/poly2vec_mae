...
python gen_traindata.py \
    --tri_dir /path/to/tri_shards \
    --model_dir /path/to/pretrained_model \
    --output_dir ./data/train_new_format \
    --nicft 256 \
    --batch_size 64 \
    --gpus 0,1,2,3
"""
...
def _process_one_batch(
    ...
    # 获取有效频域区域（去除 padding）
    mag_valid = recon[:, 0, : _WORKER_PIPELINE.valid_h, : _WORKER_PIPELINE.valid_w]   # [B, 1, H, W]
    ...
    # 为了生成 icft，仍然需要计算实部虚部（但不需要保存到输出）
    # 注意：icft 需要完整的频域网格（可能需要 padding），原代码已处理
    phase_valid = torch.atan2(sin_valid, cos_valid)
    raw_mag_valid = torch.expm1(mag_valid)   # 恢复原始幅值
    ...
    for sample_offset in range(len(tri_batch)):
        # 获取原始三角形顶点数据
        tri_array = tri_batch[sample_offset]
        triangles = torch.from_numpy(tri_array).float().cpu()

        record = {
            "triangles": triangles,
            "indices": indices_batch_u16[sample_offset].cpu(),
            "meta": torch.as_tensor(meta_batch[sample_offset], dtype=torch.float32).cpu(),
            "mag": mag_valid[sample_offset].float().cpu(),   # 形状 [1, H, W]
            "phase": phase_valid[sample_offset].float().cpu(), # 原始相位
        }
