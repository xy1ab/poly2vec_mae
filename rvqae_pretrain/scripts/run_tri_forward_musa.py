"""Run one single-sample RVQAE forward pass from triangle shards on MUSA."""
...
def _import_torch_for_device(requested_device: str):
    value = str(requested_device).strip() or "musa"
    if value == "cpu":
        import torch
    ...
    if value == "musa":
        value = "musa:0"
    if not value.startswith("musa"):
        raise ValueError(f"`device` must be `musa`, `musa:<id>` or `cpu`, got {requested_device!r}")
...
def _resolve_runtime_device(torch, requested_device: str) -> str:
    value = str(requested_device).strip() or "musa"
    if value == "cpu":
        return "cpu"
    if value == "musa":
    ...
    if not torch.musa.is_available():
        raise RuntimeError("MUSA is unavailable for current Python process.")
    return value


def _format_float_vector(value, digits: int = 4) -> str:
    flat = value.detach().float().flatten().cpu().tolist()
    return "[" + ", ".join(f"{item:.{digits}f}" for item in flat) + "]"


def _format_int_vector(value) -> str:
    flat = value.detach().float().flatten().cpu().tolist()
...
class _PolyRvqAePipelineMusa:
    """Minimal single-sample RVQAE inference pipeline with MUSA precision policy."""
    ...
    def __init__(
        ...
        *,
        ...
        from rvqae_pretrain.src.datasets.geometry_polygon import pad_triangle_batch
        from rvqae_pretrain.src.datasets.registry import get_geometry_codec
        from rvqae_pretrain.src.models.factory import build_rvqae_model_from_config
        from rvqae_pretrain.src.utils.config import load_config_any
        from rvqae_pretrain.src.utils.precision_musa import (
            autocast_context,
            normalize_precision,
        ...
        self._pad_triangle_batch = pad_triangle_batch

        self.config = load_config_any(config_path)
        self.model = build_rvqae_model_from_config(self.config, device="cpu", precision="fp32")
        ...
        self.runtime_precision = resolve_precision_for_device(self.device, self.precision)
        if self.runtime_precision == "fp32":
            self.model = self.model.to(self.device)
        else:
            self.model = self.model.to(
                device=self.device,
                dtype=precision_to_torch_dtype(self.runtime_precision),
            )
        self.model.eval()
    ...
    def triangles_to_images(self, triangles_list: list[np.ndarray]):
        import torch

        batch_tris, lengths = self._pad_triangle_batch(triangles_list, device=self.device)
        mag, phase = self.codec.cft_batch(batch_tris, lengths)
        return torch.cat([mag, torch.cos(phase), torch.sin(phase)], dim=1)

    def forward(self, imgs):
        with self._autocast_context(self.device, self.runtime_precision):
            return self.model(imgs, use_vq=True)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run one single-sample RVQAE forward pass by gid on MUSA or CPU.")
    ...
    parser.add_argument("--device", type=str, default="musa", help="Runtime device type: `musa`, `musa:<id>` or `cpu`.")
    parser.add_argument("--precision", type=str, default="fp32", help="Inference precision on MUSA.")
    ...
    torch = _import_torch_for_device(args.device)
    runtime_device = _resolve_runtime_device(torch, args.device)
    if __package__ in {None, ""}:
        import importlib

        trainer_module_name = (
            "rvqae_pretrain.src.engine.trainer_musa"
            if runtime_device.startswith("musa")
            else "rvqae_pretrain.src.engine.trainer"
        )
        trainer_module = importlib.import_module(trainer_module_name)
    else:
        if runtime_device.startswith("musa"):
            from ..src.engine import trainer_musa as trainer_module
        else:
    ...
    pipeline = _PolyRvqAePipelineMusa(
    ...
    with torch.no_grad():
        outputs = pipeline.forward(imgs)

    recon_imgs = outputs.recon_imgs.float()
    indices = outputs.indices.long()
    if indices.ndim == 3:
        indices = indices.unsqueeze(1)
    indices_save = indices.to(dtype=torch.int32, device="cpu")
    ...
    result_payload = {
        ...
        "precision": str(pipeline.runtime_precision),
    }
    ...
    print(f"[INFO] precision        : {pipeline.runtime_precision}")
    print(f"[INFO] meta shape       : {tuple(meta_tensor.shape)}")
    print(f"[INFO] indices shape    : {tuple(indices_save[0].shape)}")
