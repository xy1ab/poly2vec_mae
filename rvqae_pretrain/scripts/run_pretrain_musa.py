"""Pretraining launcher script for polygon RVQAE.
...
def _resolve_project_relative_config_paths(config_dict: dict, project_root: Path) -> dict:
    ...
    Args:
        config_dict: Parsed YAML config dictionary.
        project_root: `rvqae_pretrain` project root.
...
def main() -> None:
    """CLI main function for RVQAE pretraining launch."""
    ...
    if __package__ in {None, ""}:
        ...
        run_cli = importlib.import_module("rvqae_pretrain.src.engine.trainer_musa").run_cli
        load_yaml_config = importlib.import_module(
            "rvqae_pretrain.src.utils.config"
        ).load_yaml_config
    else:
