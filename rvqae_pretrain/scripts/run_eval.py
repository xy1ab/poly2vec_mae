"""Run one RVQAE reconstruction visualization for a selected shard sample."""
...
def main() -> None:
    ...
    if __package__ in {None, ""}:
        ...
        PtShardManifest = importlib.import_module("rvqae_pretrain.src.datasets.pt_manifest").PtShardManifest
        ...
        filesystem_module = importlib.import_module("rvqae_pretrain.src.utils.filesystem")
    ...
    parser = argparse.ArgumentParser(description="Run one RVQAE reconstruction visualization")
    ...
    panels = [
        ...
        (
            spatial_recon,
            f"Spatial Recon\nPerp={_format_float_vector(perplexity)}, Active={_format_int_vector(active_codes)}",
            "gray",
        ),
    ]
    ...
    output_path = output_dir / f"rvqae_eval_row_{args.row_index:06d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)

    print(f"[INFO] Saved visualization to: {output_path}")
    print(f"[INFO] Perplexity={_format_float_vector(perplexity, digits=4)} | ActiveCodes={_format_int_vector(active_codes)}")
