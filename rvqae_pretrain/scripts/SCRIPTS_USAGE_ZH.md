...
conda activate p2v
cd /home/xiaoyang/workspace/poly2vec_mae_v1/rvqae_pretrain
```
...
- 具体功能：读取 `configs/pretrain_base.yaml` 与命令行覆盖参数，启动 `optional conv stem + linear patchify + ViT + RVQ + ViT + linear unpatchify + symmetric upsampling + optional full-res head` 的 RVQAE 训练。训练输出统一写到 `<save_dir>/<run_name>/best` 与 `ckpt`。如果 `ckpt/train_state_a.pth` 或 `ckpt/train_state_b.pth` 已存在，使用同一条命令会自动续训。

- 当前实现说明：
  - encoder token 数由 `product(enc_conv_strides) * patch_size` 决定；当 `enc_conv_channels: []` 且 `enc_conv_strides: []` 时，模型退化为纯 ViT patchify 路径；
...
- RVQ 训练关键参数：
  - `codebook_size`：码本大小。
  - `code_dim`：量化空间维度。
  - `rvq_num_quantizers`：residual quantizer 层数；设为 `1` 时退化为单层 VQ。
  - `rvq_loss_weights`：各层 residual commitment loss 权重，长度必须等于 `rvq_num_quantizers`。
...
- 日志中的核心 RVQ 指标：
  - `VQ`：量化附加损失项。
  - `Perplexity`：各层码本使用多样性，格式为列表。
  - `ActiveCodes`：各层当前批次或当前评测窗口内实际被使用到的 code 数量，格式为列表。
...
- 具体功能：加载 `rvqae_best.pth`，对一个指定样本执行 `三角样本 -> 频域图 -> RVQAE 重建 -> 频域结果 -> ICFT 可视化`，并输出一张 PNG。

- 主要输入：
  - `--model_dir`：包含 `rvqae_best.pth` 与 `config.yaml` 的目录。
...
- 输入：
  - `--model_dir`：包含 `rvqae_best.pth` 与 `config.yaml`
...
python scripts/run_decode_indices.py \
  ...
  --model_dir ./outputs/exports/rvqae_bundle \
...
- 具体功能：从完整的 `rvqae_best.pth` 导出下游任务所需的组件文件。
...
python scripts/run_export.py \
  --rvqae_ckpt_path ./outputs/ckpt/20260407_1200/best/rvqae_best.pth \
  --config_path ./outputs/ckpt/20260407_1200/best/config.yaml \
  --output_dir ./outputs/exports/rvqae_bundle \
  --precision fp32
```
...
- 具体功能：遍历一批三角样本，统计当前 RVQAE 在整批数据上的码本使用情况。
