"""Runtime validation tests for training, pipeline, and legacy evaluator."""
...
from ..src.engine.pipeline import PolyMaeReconstructionPipeline
...
def _build_training_args(**overrides):
    """Create a minimal valid training-args namespace for validator tests."""
    payload = {
        "lr": 1e-3,
        "min_lr": 0.0,
        "batch_size": 8,
...
def test_pipeline_rejects_invalid_mask_ratio_before_processing() -> None:
    """Reconstruction pipeline should fail fast before touching geometry processing."""

    class _DummyModel:
        _validate_mask_ratio = staticmethod(MaskedAutoencoderViTPoly._validate_mask_ratio)

    pipeline = object.__new__(PolyMaeReconstructionPipeline)
