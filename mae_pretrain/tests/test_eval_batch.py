...
encoder checkpoint plus one full MAE checkpoint from `--model_dir`, and writes
one output `.pt` file where each sample stores:

1) Original triangle tensor `[T, 3, 2]`
2) Encoder CLS embedding `[D]`
3) MAE decoder frequency-domain real map `[H, W]`
4) MAE decoder frequency-domain imaginary map `[H, W]`

The script depends only on `mae_pretrain/src/*` modules and does not import any
launcher under `mae_pretrain/scripts`.
"""
...
from mae_pretrain.src.utils.safe_load import register_numpy_safe_globals
...
def _resolve_model_artifacts(model_dir: str) -> tuple[Path, Path, Path]:
    ...
    for root in search_roots:
        for path in [
            ...
            if path.exists() and path not in mae_candidates:
                mae_candidates.append(path)
        mae_candidates.extend(
            path for path in sorted(root.glob("mae_ckpt_*.pth")) if path not in mae_candidates
        )
    ...
    if not mae_candidates:
        raise FileNotFoundError(f"No MAE checkpoint found in model_dir: {base}")
    if not encoder_candidates:
        raise FileNotFoundError(f"No encoder checkpoint found in model_dir: {base}")

    return encoder_candidates[0], mae_candidates[0], config_candidates[0]
...
def _pred_patches_to_complex_maps(
    ...
    """Convert MAE patch predictions into valid real/imaginary frequency maps."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Build downstream samples from triangulated shards using encoder+MAE inference."
    )
...
def main() -> None:
    ...
    encoder_weight, mae_weight, config_path = _resolve_model_artifacts(args.model_dir)
    ...
    def flush_pending() -> None:
        ...
        with torch.no_grad():
            ...
            with autocast_context(device, args.precision):
                ...
                _, pred, _, _, _ = mae_model(imgs, mask_ratio=0.0)
