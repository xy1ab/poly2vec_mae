...
This script performs MAE reconstruction visualization by:
1) resolving one MAE checkpoint + config from `--model_dir`,
2) locating one triangle sample by global row index from `--data_dir`,
3) running mask-ratio controlled MAE inference, and
...
def _resolve_model_paths(
    ...
    """Resolve one MAE checkpoint and one model config from `model_dir`.
    ...
    config_candidates = [base / "config.yaml", base / "config.yml", base / "poly_mae_config.json"]
    ...
    if not checkpoint_candidates:
        raise FileNotFoundError(
            "No MAE checkpoint found in model_dir. Expected a `.pth` file whose name contains "
            "`mae` or `encoder_decoder`."
        )
...
def _build_eval_parser(project_root: Path) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--unet_model_path", type=str, required=True, help="Path to the UNet model checkpoint (.pth) for reconstruction.")
    return parser
...
def main() -> None:
    ...
    unet_model.load_state_dict(torch.load(args.unet_model_path, map_location=device))
    unet_model.eval()
    ...
    with torch.no_grad():
        ...
        #####################
        x_spatial = spatial_icft_orig.unsqueeze(1) #[1, 1, 256, 256]
        
        # 使用双线性插值, 将频域特征强制缩放到 256x256 (通道2, 通道3)
        mag_map = F.interpolate(mag_fix, size=(256, 256), mode='bilinear', align_corners=False)
        phase_map = F.interpolate(phase_fix, size=(256, 256), mode='bilinear', align_corners=False)
