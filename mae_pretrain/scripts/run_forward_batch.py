...
plus one MAE checkpoint from `--model_dir`, and writes one output `.pt` file.
...
def _resolve_model_artifacts(model_dir: str) -> tuple[Path, Path, Path, bool]:
    ...
    if not mae_candidates:
        raise FileNotFoundError(
            "No MAE checkpoint found in model_dir. Expected a `.pth` file whose name "
            "contains `mae` or `encoder_decoder`."
        )

    def _mae_rank(path: Path) -> tuple[int, int, float, str]:
        name_lower = path.name.lower()
        if name_lower == "encoder_decoder.pth":
            priority = 0
    ...
    mae_candidates = sorted(mae_candidates, key=_mae_rank)
    encoder_candidates = sorted(encoder_candidates, key=_encoder_rank)

    mae_path = mae_candidates[0]
    if encoder_candidates:
        return encoder_candidates[0], mae_path, config_candidates[0], False
    return mae_path, mae_path, config_candidates[0], True
...
def _reconstruct_complex_maps(
    ...
    """Convert MAE output into merged reconstruction real/imaginary maps."""
    import torch
    ...
    mask_map = mask_seq.reshape(batch_size, h_patch, w_patch, 1, 1).expand(-1, -1, -1, patch_size, patch_size)
    mask_map = mask_map.permute(0, 1, 3, 2, 4).reshape(batch_size, 1, img_height, img_width)

    recon_imgs = imgs * (1 - mask_map) + pred_img * mask_map
    recon_valid = recon_imgs[:, :, :valid_h, :valid_w]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mask_ratio", type=float, default=0.75, help="MAE mask ratio used for reconstruction export.")
...
def main() -> None:
    ...
    if __package__ in {None, ""}:
        ...
        shard_io = importlib.import_module("mae_pretrain.src.datasets.shard_io")
        ...
        dataset_module = importlib.import_module(
            "mae_pretrain.src.datasets.sharded_pt_dataset"
        )
        _ensure_numpy_float32 = dataset_module._ensure_numpy_float32
        factory_module = importlib.import_module("mae_pretrain.src.models.factory")
        load_mae_model = factory_module.load_mae_model
        ...
        from ..src.models.factory import load_mae_model, load_pretrained_encoder
    ...
    encoder_weight, mae_weight, config_path, encoder_fallback_to_mae = _resolve_model_artifacts(args.model_dir)
    ...
    mae_model, runtime_config = load_mae_model(
        weight_path=mae_weight,
        config_path=config_path,
        device=device,
        precision=args.precision,
    )
    ...
    metadata: dict[str, Any] = {
        ...
        "mae_weight": str(Path(mae_weight).expanduser().resolve()),
    ...
    def flush_pending() -> None:
        ...
        with torch.no_grad():
            ...
            with autocast_context(device, args.precision):
                ...
                pred, mask_seq = mae_model(imgs, mask_ratio=args.mask_ratio)
